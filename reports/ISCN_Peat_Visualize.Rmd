...
```{r CaCO3, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('^caco3', variable) & value > 0),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
  facet_wrap(type~units)

summary(subset(meta$sample, grepl('^caco3', variable) & (value <= 0)))
```
...
```{r pH, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('^ph_', variable) & 
                      value > 1.5 & value < 11),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  #scale_x_log10() + 
...
summary(subset(meta$sample, grepl('^ph_', variable) & (value <= 1.5 | value > 11)))
#d_ply(subset(meta$sample, grepl('^ph_', variable) & (value <= 1.5 | value > 11)), 
#      'variable', function(xx){print(summary(xx))})
```
...
```{r Texture, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('(wpg|tot_psa)', variable) & 
                      value < 100 & value >= 0 ),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  #scale_x_log10() + 
...
summary(subset(meta$sample, grepl('(wpg|tot_psa)', variable) & 
                 (value < 0 | value > 100)))
```
...
```{r cat, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('(cat_exch|cec_sum|ecec)', variable)& (value > 0)),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
  facet_wrap(type~units)

summary(subset(meta$sample, grepl('(cat_exch|cec_sum|ecec)', variable) & (value <= 0)))
```

```{r bs, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('(bs|bs_sum)', variable)& (value > 0)),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
  facet_wrap(type~units)

summary(subset(meta$sample, grepl('(bs|bs_sum)', variable) & (value <= 0)))
```
...
```{r metals_ext, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('(ca|k|mg|na)', variable) & value > 0),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
  facet_wrap(type~units, scales='free')

summary(subset(meta$sample, grepl('(ca|k|mg|na)', variable) & (value <= 0)))
```

## metal (Al, Fe, Mn) extracts
For meta this is only for NRCS data.
...
```{r metals_percent, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, 
                    grepl('^(al|fe|mn)_', variable) &
                      (value <= 100 & value >= 0)),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
...
summary(subset(meta$sample, grepl('^(al|fe|mn)_', variable) &
                      !grepl('ext', variable) & (value < 0 | value > 100)))
```
...
```{r p_percent, eval=FALSE, include=FALSE}
ggplot(merge(subset(meta$sample, grepl('^p_', variable)),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
  facet_wrap(type~units, scales='free')

summary(subset(meta$sample, grepl('^p_', variable) & (value > 100 | value < 0)))
```

## Isotope

```{r isotope, eval=FALSE, include=FALSE}
#15n          13c          14c
ggplot(merge(subset(meta$sample, grepl('^1[345]', variable)),
             meta$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  #scale_x_log10() + 
  facet_wrap(~type, scales='free')

summary(subset(meta$sample, grepl('^1[345]', variable)))